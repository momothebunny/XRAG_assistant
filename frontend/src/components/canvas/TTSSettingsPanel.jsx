    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3.5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-bold text-slate-100">Text-to-Speech</p>
            <p className="truncate font-mono text-[10.5px] text-slate-400">
              <span className="text-amber-400">{activeProvider.label}</span>  {payload.voice}
            </p>
          </div>
          <div className="hidden @[280px]:flex shrink-0 flex-col items-end gap-0.5 text-right">
            <span className="text-[10.5px] font-bold text-amber-400">{payload.format.toUpperCase()}</span>
            <span className="font-mono text-[10px] text-slate-400">{payload.speed.toFixed(2)}</span>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-amber-700/40 bg-amber-900/20 p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-300">Synthesis summary</p>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-300">Voice &amp; quality</h4>
        </header>
        <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div>
            <FieldLabel title="Speed" help="Playback speed multiplier (0.254.0)." />
            <input
              type="number"
              min={0.25}
              max={4}
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-700/50 bg-[#0d1117] px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        ...
        Output: <span className="font-mono text-amber-400">audio_url  spoken</span>
      </div>
    </div>
  );
}
