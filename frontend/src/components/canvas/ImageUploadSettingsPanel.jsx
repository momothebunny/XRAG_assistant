    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3.5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-bold text-slate-100">Image Upload</p>
            <p className="truncate font-mono text-[10.5px] text-slate-400">
              <span className="text-violet-400">{activeMode.label}</span>  {activeRole.label}
            </p>
          </div>
          <div className="hidden @[280px]:flex shrink-0 flex-col items-end gap-0.5 text-right">
            <span className="text-[10.5px] font-bold text-violet-400">{payload.maxFileSizeMb} MB</span>
            <span className="font-mono text-[10px] text-slate-400">{payload.autoResize ? 'auto-fit' : 'native'}</span>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-violet-700/40 bg-violet-900/20 p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-violet-300">Source summary</p>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-300">Image source</h4>
        </header>
        <div>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-300">Processing</h4>
        </header>
        <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          <ToggleChip checked={payload.autoResize}      onChange={(v) => setField('autoResize', v)}      label="Auto-resize to 20482" />
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-700/50 bg-[#0d1117] px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        ...
        Output: <span className="font-mono text-violet-400">documents[]  images[]</span>
      </div>
    </div>
  );
}
