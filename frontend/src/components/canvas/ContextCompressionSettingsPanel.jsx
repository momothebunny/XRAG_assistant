    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3.5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-bold text-slate-100">Context Compression</p>
            <p className="truncate font-mono text-[10.5px] text-slate-400">
              <span className="text-cyan-400">{activeStrategy.label}</span> · {payload.maxTokens.toLocaleString()} tok
            </p>
          </div>
          <div className="hidden @[280px]:flex shrink-0 flex-col items-end gap-0.5 text-right">
            <span className="text-[10.5px] font-bold text-cyan-400">budget</span>
            <span className="font-mono text-[10px] text-slate-400">{payload.maxCharsPerChunk}c / chunk</span>
          </div>
        </div>
        <p className="mt-2.5 text-[10.5px] leading-snug text-slate-400">
          Fits the retrieved context inside the LLM window. <span className="font-semibold text-slate-200">{activeStrategy.label}</span>{' '}
      ...
      <div className="rounded-2xl border border-cyan-700/40 bg-cyan-900/15 p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-cyan-300">Budget summary</p>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-300">Budget &amp; limits</h4>
        </header>
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-700/50 bg-[#0d1117] px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        ...
        Output: <span className="font-mono text-cyan-400">trimmed chunks</span>
      </div>
    </div>
  );
}
