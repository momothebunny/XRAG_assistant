...
const PATTERNS = {
  ...
  redactNames:       /\b[A-Z][a-z]{1,}\s+[A-Z][a-z]{1,}\b/g,
  redactAddresses:   /\b\d{1,4}\s?[A-Za-z.\- ]{3,}\b(?:\s+\d{4,5})?/g,
    ...
    <div className="space-y-3">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3.5 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-rose-800/50 to-rose-900/70 text-rose-200 ring-1 ring-rose-600/30">
            <Shield size={20} strokeWidth={2.2} />
          </div>
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-bold text-slate-100">PII Redaction</p>
            <p className="truncate font-mono text-[10.5px] text-slate-400">
              {enabledCount}/{DETECTORS.length} detectors  mask{' '}
              <span className="text-rose-400">{mask || ''}</span>
            </p>
          </div>
          <div className="hidden @[280px]:flex shrink-0 flex-col items-end gap-0.5 text-right">
            <span className="text-[10.5px] font-bold text-rose-400">privacy</span>
        ...
        <p className="mt-2.5 text-[10.5px] leading-snug text-slate-400">
          The <span className="font-semibold text-slate-200">privacy hop</span>  masks
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <span className="ml-auto rounded-full bg-rose-900/40 px-1.5 py-px text-[9px] font-bold text-rose-400">
            {enabledCount} on
          </span>
        </header>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-700/50 bg-[#0d1117] p-3">
        <header className="flex items-center gap-2">
          ...
          <button
            ...
            className="ml-auto rounded-md border border-slate-700/50 bg-[#0d1117] px-2 py-0.5 text-[10px] font-semibold text-slate-300 transition hover:border-rose-300 hover:bg-rose-900/20 hover:text-rose-400"
          >
            Reset sample
          </button>
        </header>
        ...
        <div>
          ...
          <pre className="max-h-40 overflow-auto whitespace-pre-wrap break-words rounded-lg border border-rose-700/40 bg-rose-900/20 p-2 font-mono text-[10.5px] leading-relaxed text-rose-900">
{preview || ' '}
          </pre>
        </div>
      </section>
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-700/50 bg-[#0d1117] px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
        ...
        Output: <span className="font-mono text-rose-400">chunks</span>  next-node
      </div>
    </div>
  );
}
