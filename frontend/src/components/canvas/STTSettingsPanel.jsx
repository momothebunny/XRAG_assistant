    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-bold text-slate-800">Speech-to-Text</p>
            <p className="truncate font-mono text-[10.5px] text-slate-500">
              <span className="text-amber-700">{activeModel.label}</span> · {payload.language}
            </p>
          </div>
          <div className="hidden @[280px]:flex shrink-0 flex-col items-end gap-0.5 text-right">
            <span className="text-[10.5px] font-bold text-amber-700">audio → text</span>
            <span className="font-mono text-[10px] text-slate-500">{payload.timestamps ? 'ts' : 'plain'}</span>
          </div>
        </div>
        <p className="mt-2.5 text-[10.5px] leading-snug text-slate-500">
          Transcribes incoming audio. Set language to <span className="font-mono text-slate-700">auto</span>{' '}
          to detect, or to ISO codes like <span className="font-mono text-slate-700">en</span>,{' '}
          <span className="font-mono text-slate-700">hu</span>.
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-3">
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-amber-800">Capture summary</p>
      ...
      <section className="space-y-2 rounded-2xl border border-slate-200 bg-white p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-600">Transcription model</h4>
        </header>
        <select
          value={payload.model}
      ...
      <section className="space-y-2 rounded-2xl border border-slate-200 bg-white p-3">
        <header className="flex items-center gap-2">
          ...
          <h4 className="text-[11px] font-semibold uppercase tracking-wider text-slate-600">Language &amp; format</h4>
        </header>
        <div>
...
{JSON.stringify(payload, null, 2)}
      ...
      <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-slate-500">
        ...
        Output: <span className="font-mono text-amber-700">transcript text</span>
      </div>
    </div>
  );
}
